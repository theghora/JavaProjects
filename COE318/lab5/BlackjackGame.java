...
public class BlackjackGame {
  ...
  public int score(CardPile p) {
    //We set the initial score as 0.
    int scoreval = 0;
    
    // Next, we use an enhanced for loop to loop through all the cards in the specified Card Pile.
    for (Card ca : p.getCards()){
        // If the rank of the card is Jack, Queen, or King, it will add 10 to the score, else it will add the rank value into the score.
        // (Note that int of ace is 14, but has already been converted into 1).
        if (ca.getRank() > 10)
            scoreval += 10;
        else
            scoreval += ca.getRank();
